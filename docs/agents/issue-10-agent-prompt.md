# Issue #10 Agent Prompt

Use this prompt to launch an autonomous agent for issue `#10`.

```text
You are going to implement GitHub issue #10 in FireLakeLabs/verso:

Add Listening Cadence and Runtime reports
...
Add thin first versions of the Listening Cadence Heatmap and Runtime Distribution reports using frontend TypeScript transform modules and isolated report views. Listening Cadence should use purchase timestamps plus current completion state only and must not invent missing listen-session history.

## Acceptance criteria

- [ ] Listening Cadence shows a 52-week purchase heatmap and purchase-cohort completion summary without implying missing session history.
- [ ] Runtime Distribution shows runtime bins, median/mean/P90 markers where data exists, and long/short outlier tables.
- [ ] Missing Datapoints remain null unless the display explicitly labels a derived presentation.
- [ ] Frontend transform modules are testable without rendering React components.
- [ ] Report UI is reachable from Library Overview.
- [ ] Tests cover cadence bucketing, cohort completion rollups, and runtime binning.
...
- Use a dedicated repo-local worktree for this issue:
  - branch: `issue-10-cadence-runtime`
  - worktree: `.worktrees/issue-10-cadence-runtime`
...
- Use issue-derived ports for all local runs and browser checks:
  - frontend: `5210`
  - backend: `7210`
- Prefer root orchestration commands such as `just dev ISSUE=10`, `just frontend-test`, `just frontend-visual FRONTEND_PORT=5210`, and `just verify`.
...
- Because issue #10 changes a signed-off frontend surface, treat visual parity updates as part of the implementation, not cleanup. If the UI changes intentionally, update the relevant parity states/baselines and explain the change in the PR.
...
- Add frontend transform tests for cadence bucketing, cohort completion rollups, and runtime binning, matching the harness requirement for issue #10.
...
9. A pull request is opened from `issue-10-cadence-runtime` and is ready for review.
