# Issue #14 Agent Prompt

Use this prompt to launch an autonomous agent for issue `#14`.

```text
You are going to implement GitHub issue #14 in FireLakeLabs/verso:

Add Cost-Per-Hour report
...
Add the Cost-Per-Hour Dashboard as a thin first version using the configured Cost Basis behavior. The report should let the Library Owner toggle list-price and configured per-credit valuation, defaulting to per-credit value once configured.

## Acceptance criteria

- [ ] Cost-Per-Hour report uses explicit Cost Basis settings.
- [ ] Configured per-credit value is the default once present, while list price remains available.
- [ ] Unknown or missing cost Datapoints are not guessed silently.
- [ ] Report includes a charted cost-versus-runtime view plus ranked best-value and high-cost short-list views.
- [ ] Report UI is reachable from Library Overview.
- [ ] Frontend tests cover cost calculations, basis toggling, and missing-cost behavior.
...
- Use a dedicated repo-local worktree for this issue:
  - branch: `issue-14-cost-per-hour`
  - worktree: `.worktrees/issue-14-cost-per-hour`
- If the worktree does not exist yet, create it from the repository root with:
  - `git worktree add .worktrees/issue-14-cost-per-hour -b issue-14-cost-per-hour main`
...
- Use issue-derived ports for all local runs and browser checks:
  - frontend: `5214`
  - backend: `7214`
- Prefer root orchestration commands such as `just dev ISSUE=14`, `just frontend-test`, `just frontend-visual FRONTEND_PORT=5214`, and `just verify`.
...
- Because issue #14 changes a signed-off frontend surface, treat visual parity updates as part of the implementation, not cleanup. If the UI changes intentionally, update the relevant parity states/baselines and explain the change in the PR.
...
- Add frontend transform tests for Cost Basis calculations, basis toggling, and missing-cost behavior, matching the harness requirement for issue #14.
...
9. A pull request is opened from `issue-14-cost-per-hour` and is ready for review.
